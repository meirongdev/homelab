...
- **第 4 层**：`取不到正文` 那批是硬下限——PDF 无文本层、EPUB 无 spine 正文，
  再跑多少轮都一样。
- **客观下限**：2026-08 实测约 200 本书**在任何公开书目数据库里都不存在**
  （自出版 / Kindle 独占，无 ISBN、从未被收录）。它们只能靠第 4 层，或者接受缺失。
