...
| 11 | **readlist 公开面的私人文档清理（已公开，风险已接受）** | 2026-08-06 按库主人决定**已公开** `readlist.meirong.dev`。遗留清理项：`/api/v1/catalog` 把全部 2046 个 work 的 title+author 原样发给浏览器，而书库里混着非书的私人文档 —— 实测 1 条第三方简历（`(BHRC) Backend Engineer - 吳幸慈, Mint Wu - 202603`，author 字段是另一个人名）。「只发元数据不发书」对**书**成立，对误入库的私人文件不成立：简历的元数据本身就是真人姓名，且这类东西没有静态判据、只能靠人看。清理任一即可，**随时能做、不必先下线**：① 把该文档从 calibre 挪走 → 重跑 snapshot（约两分钟，最省事）· ② readlist 侧加准入过滤（按 tag/书架/格式，上游改动）· ③ 公开面只发榜单、不发全量 catalog（上游改动）。⚠️ **硬前置**：上游 `review-2026-08-05-b` 的 C1 提议给 catalog 做 SSR + sitemap —— 那个落地前必须先做完①②③之一，否则这条会从「悄悄可见」变成「被搜索引擎收录」。 |
