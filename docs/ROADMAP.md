...
| 13 | **PSA 收尾（zitadel → restricted + 清单外 ns）** | 2026-08-10 补上了 `zitadel` ns 的 PSA 标签（此前**一个都没有**，吃内置默认 privileged 且 warn/audit 全空，实测特权 Pod 可建成且零 warning），现为 `baseline` + warn/audit `restricted`。剩两件：① **翻 restricted** 需先在 [`zitadel.yaml`](../cloud/oracle/manifests/zitadel/zitadel.yaml) 的 `valuesContent` 补 `securityContext`/`podSecurityContext`（缺 `allowPrivilegeEscalation:false`/`capabilities.drop[ALL]`/`seccompProfile`；chart 9.34.1 已用 `helm template` 验证这两个键会落到全部 9 个容器）——**顺序反了会在下次 chart 升级时把 `zitadel-init`/`zitadel-setup` Job 挡在准入外**；改 values 本身会触发 helm upgrade 重跑那两个 Job，`auth.meirong.dev` 短暂不可用，要挑窗口。② **清单外的 ns 没人管**：oracle 的 `external-secrets`/`cnpg-system`/`trivy-system`/`default`/`cilium-secrets` 由 Helm/ArgoCD `CreateNamespace` 建、不在任何清单里，故 CI 的 H5 也查不到；补它们要先定下发方式（oracle 侧 `just` 任务 vs Application 的 `managedNamespaceMetadata`）。⚠️ `trivy-system` 别照 dry-run 干净就定 baseline——node-collector 是周期 Job 且要 hostPath，homelab 侧定的就是 privileged。（[security.md §5.1](reference/security.md)） |
| 14 | **低优先 / 可选** | Renovate（chart/image 版本自动 PR）· MacBook `TargetDown` 静默规则 · Vault Dynamic Secrets（PostgreSQL 动态凭据，规模不需要）· Cloudflare Pro WAF（Managed Ruleset + OWASP CRS）。（母文档 P2） |
...
| 2026-08-08 | **Bifrost 退役**：整个 `bifrost` ArgoCD App（LLM 网关 + oauth2-proxy + dgx-proxy）删除；LLM 网关由 LiteLLM 接替（[计划](plans/apps/2026-08-01-litellm-gateway-migration.md)，尚未落地） |
