...
| 13 | **集群外三站点的可用性监控** | 🚧 **已实现，待 push 生效**。`stack.meirong.dev`（另一个仓库 home-stack 部到 Cloudflare Workers，见 [决策](decisions/home-stack-repo-boundary.md)）、`playgrounds.meirong.dev`、apex 博客三个此前**一个监控都没有**，即"站点挂了没人知道"。已加：uptime-kuma provisioner 的 `MONITORS` 三条（探 `/`，实测三者均 200 无跳转）+ Home Stack 的 Homepage 磁贴（另两个早就有磁贴）。⚠️ GitOps：本地 commit 不生效，**push 后** ArgoCD 同步 → PostSync hook 重跑 provisioner 才建监控；验收看 `status.meirong.dev` 上是否出现三条，或 `kubectl --context oracle-k3s -n personal-services logs job/uptime-kuma-provisioner`。⚠️ 这三条**不能**当集群存活信号（不经集群、不经隧道），dead-man's switch 仍只认 Grafana/Vault/Open Notebook |
