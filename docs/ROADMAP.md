...
| 13 | **`stack.meirong.dev` 没有可用性监控** | 2026-08-23 上线的集群外站点（另一个仓库 home-stack 部到 Cloudflare Workers，见 [决策](decisions/home-stack-repo-boundary.md)）。按那份归属表**可用性监控归 homelab**，但它目前既没有 Uptime Kuma 监控项、也没有 Homepage 磁贴 —— 站点挂了不会有人知道。⚠️ 它不经隧道也不经集群，所以探测就是普通 HTTPS GET；注意 404 页现在返回真 404（别把探测路径写成不存在的路径）。同类的 `playgrounds.meirong.dev`、apex 博客也没有 —— 一并补更划算 |
