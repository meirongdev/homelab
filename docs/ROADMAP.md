...
| 13 | **zitadel ns 上 restricted（PSA 收尾最后一步）** | 2026-08-10 补上了 `zitadel` ns 的 PSA 标签（此前**一个都没有**，吃内置默认 privileged 且 warn/audit 全空，实测特权 Pod 可建成且零 warning），现为 `baseline` + warn/audit `restricted`。**同日已做完的**：清单外 ns 全部补齐（homelab 的 `personal-services`——08-03 级联删除后重建、标签丢了近一周——加 `external-dns`/`opencost`；oracle 的 `default`/`external-secrets`/`cnpg-system` 定 baseline、`kube-system`/`trivy-system` 定 privileged），两集群各加 `just psa-check` 漂移哨兵，并全量核过 124 个 Pod、无一违反自己 ns 的档。**剩这一件**：翻 restricted 前必须先在 [`zitadel.yaml`](../cloud/oracle/manifests/zitadel/zitadel.yaml) 的 `valuesContent` 补 `securityContext`/`podSecurityContext`（现缺 `allowPrivilegeEscalation:false` / `capabilities.drop[ALL]` / `seccompProfile`；chart 9.34.1 已用 `helm template` 验证这两个键会落到全部 9 个容器，不是 `timeoutSeconds` 那种被静默丢弃的键）。**顺序反了会在下次 chart 升级时把 `zitadel-init`/`zitadel-setup` Job 挡在准入外**，升级卡在 DB 迁移中途；而改 values 本身就会触发 helm upgrade 重跑那两个 Job，`auth.meirong.dev` 短暂不可用——所以要挑窗口，不能顺手带过。（[security.md §5.1](reference/security.md)） |
