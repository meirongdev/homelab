...
| 14 | **opsx/OpenSpec 装了但产物不落库** | 同一批 4 个 `SKILL.md` 在 `.agent` `.claude` `.codex` `.gemini` `.qwen` 各存一份（5×4），而 `openspec/` 整体 gitignore，本地 `specs/` 空、`changes/` 只剩空 `archive/`——提案写完即丢，等于没发生过。本仓库设计产物只认 `docs/plans/` + `decisions/` 一条路。**两种收法都成立**：停用 opsx，或只留一个 agent 目录并把 `openspec/changes/` 纳入 git。维护成本实测在涨（`.qwen/commands/` 已混进 `.md`+`.toml`+`.toml.backup` 三种形态）。边界全文见 [agent-tooling.md](reference/agent-tooling.md) |
| 15 | **`sync-ebooks` 两份实现合一** | `scripts/sync-ebooks.sh`（577 行，重试/退避、并发锁、模糊去重、清理、备份、sha256、tar 传输，`docs/guides/ebook-sync.md` 认它）与 `.claude/skills/sync-ebooks/scripts/sync_ebooks.py`（258 行，全参数化、`--timeout`/`--cp-timeout`、按 `status.phase=Running` 选 pod）在仓库里并存，两边都还在被改。**两份都还能用，所以先不删**——要合的是能力不是文件：以 bash 为主实现，把 python 的超时与 Running 过滤并进去，python 那份退成调用壳。一个流程两份实现的边界见 [agent-tooling.md](reference/agent-tooling.md) |
