...
| 11 | **readlist 重新公开的前置（2026-08-05 上线，公开入口暂摘）** | 服务/PVC/CronJob/指标/告警**全部在跑**，只有 HTTPRoute + 首页条目被注释掉。拦路的是一件**只能由你定**的事：`/api/v1/catalog` 会把全部 2046 个 work 的 title+author 原样公开，而书库里混着非书的私人文档 —— 实测至少 1 条第三方简历（`(BHRC) Backend Engineer - 吳幸慈, Mint Wu - 202603`，author 字段是另一个人名）。「只发元数据不发书」对**书**成立，对误入库的私人文件不成立：简历的元数据本身就是真人姓名，而这类东西没有静态判据、只能靠人看。三个选项：① 把非书文档从 calibre 挪走再重跑 snapshot（最省事）· ② readlist 侧加准入过滤（按 tag/书架/格式，属上游改动）· ③ 公开面只发榜单、不发全量 catalog（上游改动）。定了之后取消 [`readlist.yaml`](../cloud/oracle/manifests/personal-services/readlist.yaml) 里那段路由的注释即可。⚠️ external-dns 是 upsert-only，摘路由没删 CNAME，`readlist.meirong.dev` 仍解析（当前 404）。 |
