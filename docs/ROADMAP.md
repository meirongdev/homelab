...
| 11 | **calibre-web 迁 oracle-k3s** | homelab 磁盘 65%（剩 43.5 GB）唯一的实质解法：书库 23G + config 15G = **38 GB**，arm64 镜像已核实可用，oracle 剩 157 GB。2026-08-02 迁移盘点里收益最大但未做的一笔。（[盘点 §2](plans/architecture/2026-08-02-homelab-to-oracle-workload-migration.md)）|
| 12 | **低优先 / 可选** | Renovate（chart/image 版本自动 PR）· MacBook `TargetDown` 静默规则 · Vault Dynamic Secrets（PostgreSQL 动态凭据，规模不需要）· Cloudflare Pro WAF（Managed Ruleset + OWASP CRS）。（母文档 P2） |

### 已知问题（不阻塞，无人认领）

- **`argocd-image-updater` App 已装未用**：chart 1.2.4 在跑但 0 个 `ImageUpdater` CR，oracle-k3s App 只有旧式注解、无对应 CR——实际没在更新任何镜像。要么补 CR 接上，要么退役（见 `docs/decisions/argocd-image-updater.md`）。**2026-08-02 起它随 ArgoCD 控制面运行在 oracle-k3s**，迁移刻意保持现状未做取舍。
