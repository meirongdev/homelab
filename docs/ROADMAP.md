...
| 7 | **oracle external-dns 告警覆盖** | ⚠️ **2026-08-20 复核修正**：本条原写"需 oracle OTel 抓 `:7979`"，**那一半早已做好** —— oracle otel 的 `prometheus/external-dns` receiver 在抓，中枢 Prometheus 里 `external_dns_controller_last_sync_timestamp_seconds{cluster="oracle-k3s"}` 有数据。真正剩下的只有**规则侧**：`manifests/monitoring/alerts/external-dns-alerts.yaml` 的 4 条仍写死 `cluster="homelab"`，oracle 实例挂了不会响。改法 = 去掉该 label 选择器或改成 `cluster=~".+"`，并确认 absent 类规则按 cluster 分组。非阻塞。（[external-dns 决策](decisions/external-dns-adoption.md)） |
| 14 | **capacity 组两条告警只覆盖 homelab，而 oracle 更紧** | `HomelabCpuRequestsSaturated` / `HomelabMemoryRequestsSaturated` 写死 `cluster="homelab"`。清单注释给的理由是"oracle 的序列不进中枢 Prometheus"，**2026-08-24 复核已不成立** —— `count by (cluster)` 显示 `kube_node_status_allocatable` 与 `namespace_{cpu,memory}:kube_pod_container_resource_requests:sum` 两边都有序列。按 cluster 分组实测：**CPU homelab 26.6% / oracle 60.1%、内存 homelab 61.5% / oracle 73.6%** —— 逼近 90% 阈值的恰是没被覆盖的那个，且 oracle 是**单向缩容过**（4 OCPU/24GB→2/12，A1 无容量涨不回去）的集群。改法 = `sum()` 换成 `sum by (cluster)` 并重命名（现名与 annotation 里"迁负载去 oracle"的建议都是 homelab 专属，对 oracle 讲不通）。⚠️ 分母仍必须保留 `job="kube-state-metrics"`（opencost 吐同名指标，实测两集群各 2+2 series，不过滤则分母翻倍）。与 #7 同一类问题：**规则侧写死了集群**。（[reference/cronjob-fleet.md](reference/cronjob-fleet.md) 附近的 [告警覆盖](reference/observability-alerting-slo.md)）
