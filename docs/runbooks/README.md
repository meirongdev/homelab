...
| [add-service.md](add-service.md) | 往任一集群部署新服务：定落点 → 清单 → HTTPRoute（=改 DNS）→ GitOps 注册 → homepage/Uptime Kuma → 验收；含 ☠️ PVC 备份归属、新 ns 的 PSA、ReferenceGrant 版本、路由竞态四类静默失效。skill `add-service` 的真相源 |
