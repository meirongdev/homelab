...
1. **`zfs-dkms` 会在本机现编一份 `zfs.ko`**，与 `proxmox-kernel-*` 自带的那份打架。
...
| 重启 `storage-106` | VM 200 `k3s-exp`（= 节点 `k8s-worker-106`）下线，外加 5 个只读媒体 NFS、restic 备份目标、`pve` 的 `backups` 存储（它是 106 的 NFS 导出）一起没 |
