...
```bash
kubectl -n personal-services exec deploy/calibre-web -c calibre-web -- sh -c \
...
编辑 `k8s/helm/manifests/personal-services/open-notebook-ingest.yaml` 顶部的
...
kubectl -n personal-services get cm open-notebook-ingest-params -o jsonpath='{.data.BOOK_PATTERN}'
```

### 3. 手工触发

```bash
kubectl -n personal-services create job --from=cronjob/open-notebook-ingest ingest-$(date +%s)
kubectl -n personal-services logs -l job-name=ingest-<时间戳> -f
```
...
kubectl -n personal-services exec deploy/open-notebook -- sh -c \
...
| 每本 `FAILED (401)` | Bearer 口令不对：secret 卷权限（曾因 0400 + uid 100 读不到，已改 0444）或 Vault 里换了口令没 rollout restart | `kubectl -n personal-services get secret open-notebook-secret`；对照 [reference/open-notebook.md 运维备忘](../reference/open-notebook.md) |
