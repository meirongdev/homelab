...
- 相关：`reference/observability-alerting-slo.md`（告警路由细节）、`reference/security.md` §9。
...
`cloud/oracle/cloudflare/terraform.tfvars` 的 `notify` ingress rule 条目已移除并 **apply**（同目录 `.env` 里另有一个仍有效的 `CLOUDFLARE_API_TOKEN`，`terraform.tfvars` 里那份当时已失效。两者是两份独立凭据，`just plan/apply` 通过 `dotenv-load` 只认 `.env` 那份，`tfvars` 里的值对 `just` 工作流其实是死代码，已同步更新成同一个有效 token 避免下次直接跑裸 `terraform` 命令踩坑）。`terraform plan` 确认 `No changes`，`notify.meirong.dev` 已不可达（DNS record + tunnel ingress rule 均已从 Cloudflare 侧摘除）。
