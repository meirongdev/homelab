# LLM 网关从 Bifrost 迁移到 LiteLLM（双自托管来源）
...
homelab LLM 网关（llm.meirong.dev）原为 **Bifrost**（`maximhq/bifrost`）。其配置（enforce 开关/路由/
虚拟 key）存 PVC-SQLite，管理面无鉴权需 oauth2-proxy+ZITADEL 配套，且 Bifrost 实际未被使用，已于
2026-08-08 独立退役。

用户考虑换 LiteLLM 的契机是"Rust 重写"，但核实：截至 2026-08-01 Rust 网关只覆盖 `/v1/realtime`，
生产 proxy 仍是 Python。

本仓库有两个**自托管推理源**，都可从 homelab master 直连（Open Notebook 已生产验证接线）：
...
- 用 **LiteLLM proxy**（v1.94.1，官方 Python proxy）替换 Bifrost，hostname 不变（`llm.meirong.dev`）。
...
- **砍掉 oauth2-proxy/ZITADEL client**：LiteLLM 管理面**自带认证**（`UI_USERNAME/UI_PASSWORD`，
  Vault→ESO），符合 security.md「自带认证」矩阵。备选（保留 oauth2-proxy 复用 bifrost-admin）因双重登录
  +维护面被否决。
...
- 网关**必须钉在 master（control-plane）**：只有 master 的 netmap 里有 DGX/Mac，worker-106 是
  tagged-device 看不到这两个源；钉在 worker 既连不上上游、又会在 3G VM 上 OOM（实测）。
- Python proxy 内存占用比 Bifrost（Go）高：limit 需 ≥2Gi（1Gi 实测首启跑 Prisma migration 时 OOMKilled）。
- 双自托管来源 fallback（替代原"双 DGX"设想）由 LiteLLM 原生 `fallbacks` 承载。
- 消费方（本机 codex）从 `--profile bifrost` 迁移到 `--profile litellm`，`BIFROST_VK` → `LITELLM_VK`；
