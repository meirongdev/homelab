...
`le` = **l**ess than or **e**qual，直方图分桶的上界。histogram 不存原始观测值，只存
...
（`.data.seriesCountByMetricName`），**别用** `topk(20, count by (__name__)({__name__=~".+"}))`：那是全 head 扫描。设计取舍与两个 PromQL 陷阱见
