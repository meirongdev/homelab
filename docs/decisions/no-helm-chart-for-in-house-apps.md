...
| 目录 | 内容 | 举例 |
|------|------|------|
| `k8s/helm/values/` | **全部**是上游第三方 chart 的 values（homelab 集群） | kyverno、trivy、vault、external-secrets、kube-prometheus-stack、opencost、sloth、argocd |
| `cloud/oracle/values/` | 同上（oracle 集群，2026-09-04 起独立成树） | loki、tempo、falco、cnpg-operator、trivy、external-secrets、cilium |
