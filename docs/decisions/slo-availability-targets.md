...
2026-06-16（`d5caa50`）上线 Sloth + 5 条「可用性 99% / 30d」SLO。**机制层文档一直是完整的**：指标来源、SLI 公式、兜底推导、告警映射见
...
| **uptime-kuma** | 9742 | ~9435（死人开关心跳，非探针，见下） | ~307 | ❌ |
...
⚠️ **uptime-kuma 是第三类，别按上面两类读**（2026-08-14 修正）：它没有外部探针
（Uptime Kuma 探自己走 ClusterIP，不过 gateway），但 homelab Alertmanager 的
**Watchdog 死人开关心跳**恰好推到 `status.meirong.dev/api/push/...`，
`repeatInterval: 30s`、实测约 45s 一条（08-13 全天 1887 条）。
所以它那 9742 里 ~97% 是**机器流量**，人访问只有 ~307。
本文初稿把它记成"第二大真实流量后端"是错的，已改正。
...
**维度一 · 真实流量**（去掉探针基线与承载性机器流量后）≥ 1000 请求/窗口。
...
| 流量几乎全是承载性机器流量（死人开关心跳这类） | **建定向告警盯那条链路本身，不建 SLO** |
| 有流量但没人在意 | 不建 |

第二行是关键：**关键性不是建 SLO 的理由，可测量性才是**。zitadel 是全站 SSO 单点，
但 44 个请求上的 99% 只会产生假信号；它需要的是存活探测（2026-08-12 已补上），不是 SLO。

第三行是 2026-08-14 追加的，来自 uptime-kuma 的实例（详见
[records/2026-08-14-oracle-reboot-loop-and-blind-dead-mans-switch.md](../records/2026-08-14-oracle-reboot-loop-and-blind-dead-mans-switch.md)）。
它与第二行的区别在于**错误有没有后果**：探针失败只是少一个样本，而死人开关心跳
推不进去意味着**安全网当场失效**。但这依然不该做成 SLO：
比率型 SLI 会把"每天 10 分钟全失败"稀释成 1.4% 的月度数字，
而真正要回答的问题是「**此刻这条链路通不通**」。所以用定向告警
（`DeadMansSwitchReceiverDown`），不用错误预算。
...
2. **探针基线** = 1440 × 窗口天数（仅对有 `https://*.meirong.dev` 外部探针的服务；
...
1. ☠️ **N/A / NaN 不等于健康**。预算数无效时按「未知」处理，不按「没问题」处理。
...
**~~反向缺口~~：`uptime-kuma` 那条已排查，结论与初稿相反**（2026-08-14）。
初稿据"9742 请求/5d、错误率 1.376%"判定它两维都满足、是最该补的一条 SLO。
排查后三点全部推翻：

- 那 9742 里 ~97% 是**死人开关心跳**（每 45s 一条），人访问只有 ~307 → 维度一不合格；
- 那 134 个 5xx **不是应用错误**，是 oracle 节点 6 天硬重启 6 次期间网关的 503
  突发（6 段爆发，与重启时刻逐一对齐），把它们摊成"1.376% 稳定错误率"是读错了；
- 真正的风险也不是"预算超支"，而是**重启窗口里死人开关静默失明**
  （实测 9分40秒心跳缺口、`important` 翻转 0 条）。

→ 处置改为 D2 第三行：**不建 SLO，建定向告警**。已落地 `DeadMansSwitchReceiverDown`
+ `NodeRebootLoop`。全过程见
[records/2026-08-14-oracle-reboot-loop-and-blind-dead-mans-switch.md](../records/2026-08-14-oracle-reboot-loop-and-blind-dead-mans-switch.md)。

> 这一条留着不删：它是"**比率型 SLI 会把突发中断稀释成温和百分比**"的现成教材，
> 也是本 ADR 的判据第一次被自己的数据修正。

### 遗留（刻意不做）

- **不调整现有 5 条 SLO**。删 SLO 会断掉告警名与历史序列连续性。
  （初稿在此处写的"等弄清 uptime-kuma 的 5xx"已完成，见上；结论是不需要动 SLO 清单。）
