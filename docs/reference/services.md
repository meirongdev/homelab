...
| 家庭游戏大厅 (Godot Web) | homelab | `personal-services` | `game.meirong.dev`（静态 Godot 4 Web 导出 + nginx-unprivileged:8080，`/healthz` 探活）。**无状态无 PVC**，游戏状态全在 Nakama。镜像与游戏源码在 `meirongdev/godot-games`（契约见 https://github.com/meirongdev/godot-games/blob/main/docs/deployment-contract.md ），本仓库只拥有域名/限额/落点。☠️ **这个域名下还挂着 Nakama 的 API**：Web 客户端从页面自身来源推导服务器地址（镜像里零环境事实），所以 `game.meirong.dev` 的 HTTPRoute 有三条规则 —— `/v2/*` 与 `/ws` 打到 `nakama:7350`，其余给静态站。**少了那两条游戏就是打不开**，而上游 e2e 工具直连 `nakama.meirong.dev:443`、结构上绕过它们，**e2e 全绿也证明不了网页能玩** —— 唯一判据是用浏览器真开一次（含"看得见中文"）。⚠️ 上游有个自动化替身可以直接打线上：`NAKAMA_HOST=game.meirong.dev NAKAMA_PORT=443 NAKAMA_TLS=1 python3 tools/web_smoke.py https://game.meirong.dev`（真浏览器：登录 → 大厅 → 点进别人的房间 → 退出 → 自己建房）。☠️ **那三个环境变量不是可选的**：脚本要先用 `e2e_match` 建一个"别人的房"供浏览器点，而 `e2e_match` 的 `NAKAMA_HOST` 默认是 `127.0.0.1:7350` —— 漏了就把房建到**本机 compose** 上，浏览器在线上列表里永远看不到它，报「房间列表里没有『别人的房…』这一行」，看着像线上路由/房间列表坏了（2026-08-28 实测误判过一次；补上后两档全绿）。**必须无头或让标签页可见** —— Chrome 对隐藏标签不跑 `requestAnimationFrame`，而 Godot 主循环挂在 rAF 上，标签一隐藏引擎就不转，任何等待都超时，看着像服务端卡住。⚠️ server key 是**公开常量** `family-lobby-2026`（跟着 web 制品发布），刻意放在 git 里而不是 Vault：它必须与上游 `NakamaConfig.SERVER_KEY` 一致，放 Vault 会让这个一致性看不见 |
