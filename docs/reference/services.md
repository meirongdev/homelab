...
| Scalar (API 文档) | oracle-k3s | `personal-services` | `apidocs.meirong.dev` | 2026-09-26 上线；无状态。文档由各服务自己伺服，HTTPRoute 把 `/specs/<app>.yaml` 同源转发过去；上游默认的遥测 / proxy.scalar.com / 字体 CDN / AI agent 已全关，见清单文件头 |
| Timeslot | oracle-k3s | `personal-services` | `slot.meirong.dev` | 2026-09-02 起整组资源进 kustomize 树由 ArgoCD 接管（此前只有 HTTPRoute 在 git，Deployment 是清单外 manual-helm）。管理口令 09-03 已从 chart 默认值轮换为 Vault `oracle-k3s/timeslot` 的随机值，经 ESO 下发 |
...
⚠️ 这三个不在上表里，也查不到 HTTPRoute（`kubectl get httproute` 核对不到不等于没上线），
...
| Home Stack（homelab 自托管技术选型目录，97 条） | Cloudflare Workers（Rust→wasm SSR + 静态资源层），repo `meirongdev/home-stack` | `stack.meirong.dev`（2026-08-23 上线；DNS 记录由 Workers 自定义域名自建，声明在 home-stack 仓库的 terraform，**不在本仓库 state 里**） |
...
看不见刚建的监控（客户端列表靠服务端推事件填），于是状态页的 `publicGroupList`
**落后一次运行**：监控确实建成、告警也会响，可公开状态页上一条都没有，而日志三条全绿、
ArgoCD hook 报 Succeeded，无处可查。现已改为「等所有声明的监控都出现在服务端返回里再存
状态页」，等不到则响亮警告但不失败（这是 PostSync hook，失败会连累整个 App 的 sync）。
...
2026-08-18 上线；官方 OCI Helm chart，本仓库唯一的 OCI chart 源。
...
HTTPRoute 有三条规则：`/v2/*` 与 `/ws` 打到 `nakama:7350`，其余给静态站。
...
`e2e_match` 的 `NAKAMA_HOST` 默认是 `127.0.0.1:7350`，漏了就把房建到本机 compose 上，
