...
⚠️ **残留风险（尚未处理）**：取最后一页只免疫**向上**的尖峰，对**向下**的凹陷仍暴露 ——
若某次凹陷正好落在最后一页，`scanned` 会显著大于 `total`，`absInt` 一取绝对值同样跳闸，
方向相反的同类误伤。窗口比原来窄得多（1 页 vs 831 页中任意一页），且现在
`close_skipped` 与偏差都有指标，误跳看得见、查得出。**更彻底的做法是把闸门改成
「有没有翻到空页」** —— 分页只在遇到空页或熔断时才停，翻到了空页就说明沿途那块板子
已经枚举完整，而熔断本来就单独判。留作后续。
