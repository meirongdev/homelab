...
- **`Watchdog` 不丢弃**：`watchdog` AlertmanagerConfig CRD 抢先把它 webhook 到 oracle
  Uptime Kuma 的 push monitor，即死人开关；静态路由树的 `null` receiver
  只接真正没人认领的告警。
...
- **ClusterMesh 告警**（`manifests/monitoring/alerts/clustermesh-alerts.yaml`，2026-08-05 新增）:
  当天发现跨集群 mesh 双向断开且零告警，事后连"断了多久"都无法确定：`cilium_kvstoremesh_*`
...
- **readlist 数据新鲜度告警**（`manifests/monitoring/alerts/readlist-alerts.yaml`，2026-08-05 新增，
  ...
  2026-08-09 补两条判别力规则（起因：C 维上线四天恒为 0、F 维被 snapshot 覆写
  ...
  ⚠️ 两条踩过的坑，改这个文件前先读：① 跨指标比较必须 `ignoring(grade)`，
...
- **Falco 自身健康告警**（`manifests/monitoring/alerts/falco-alerts.yaml`，2026-08-10 新增，5 条）:
  ...
  两条存活（`FalcoDown` critical / `FalcosidekickDown` warning，分开报是因为处置不同：
  ...
  ⚠️ 三条踩过或绕过的坑：① 存活规则一律写成 `up == 0 or absent(up{...})`，采集端
  （OTel Collector）挂掉时序列整体消失，`up == 0` 匹配的是空向量、永不触发；
  ② 两个前缀不可混用：引擎是 `falcosecurity_falco_*`/`falcosecurity_scap_*`，转发器是
  `falcosecurity_falcosidekick_*`；③ `status="error"` 序列首次失败后才存在，
  「查不到数据」= 正常，存活性不靠这个计数器兜底。
  上线前逐条在 live Prometheus 上双向验过（`absent()` 对存在的序列返回空、对不存在的
...
- **Prometheus 基数看门狗**（`manifests/monitoring/alerts/prometheus-rules.yaml` 的
  ...
  ⚠️ 三处特意为之：① **表达式不按 cluster 拆**，它量的是本机 head 的总内存压力，
  ...
  instance），对裸指标做 min/avg 会读到已消失副本的陈旧值。③ 排查时不要用
...
- **homelab 调度容量饱和**（`prometheus-rules.yaml` 的 `capacity` 组，2026-08-13 新增，2 条）:
  ...
  **关掉的理由是语义在本拓扑下恒真**：上游那两条量的是「挂掉最大节点后剩余容量能否装下
  ...
  清单注释当时给的理由是"oracle 的序列不进中枢 Prometheus"，复核发现已不成立：
  ...
  📌 `for: 30m` 是必需的，不是保守：oracle 内存曾达 85.2%，但只持续 5 分钟，
  那是 rollout 期间新旧 pod 的 requests 并存造成的尖峰。而 >75% 有过 115 分钟连续，
  所以 90% 阈值既够不着又不会哑火。
...
- **ResourceQuota 逼近上限**（`prometheus-rules.yaml` 的 `capacity` 组，2026-08-24 新增，1 条）:
  ...
  | `KubeQuotaExceeded` | `>1` | warning | 准入控制保证 used 最多等于 hard → **结构上永不触发** |

  ☠️ 为什么这个盲区特别坏：配额满了之后准入层静默拒绝建 pod，Job 对象建得出来、
  ...
  ② 与 `capacity` 组其它规则不同，这条不限定 `cluster="homelab"`：oracle 的
  ...
  📌 配套的口径修正：`count/pods` 是对象计数配额、不排除终态，`pods` 才排除。
...
**2026-08-02 已补齐**（`prometheus-rules.yaml` 的 `metal-nodes-resources` 组），且刻意不做
统一放宽 `job=~"node-exporter.*"`：几台机的正常态差别太大，统一阈值必然误报或漏报，
改成按主机定阈值。两个实测结论：

- **pve 的 ZFS ARC ≈ 0**（`node_zfs_arc_size` ≈ 0），低 `MemAvailable` 是虚拟机分配
  （K3s VM 独占 12.6GiB/15GiB）而非可回收缓存，且已用 1.07G swap → pve 用绝对下限
...
- **文件夹**: `folderAnnotation: grafana_folder` + `provider.foldersFromFilesStructure: true`。
  ...
  - ⚠️ **归档不等于禁用**: `nodeExporter.operatingSystem.{aix,darwin}.enabled: false` 只 gate
...
- **一手指标 (oracle)**: 没有 Prometheus Operator，otel-collector 的 `prometheus/cilium-envoy`
...
- **SLO 定义**: `manifests/monitoring/slos.yaml`，两个 `PrometheusServiceLevel`：
  ...
  （error=5xx, total=全部）。新增或修改：在对应 `spec.slos[]` 追加
...
- **☠️ 分子分母都要兜底，两个坑独立且症状完全不同**（现有 5 条已统一加固，新增必须沿用；
  ...
  - **errorQuery 末尾 `OR on() vector(0)`（2026-07-12 踩坑）**: envoy 按响应码类
    惰性创建序列，服务从未返回过 5xx（或 envoy 重启计数器重置）时 errorQuery 为
    空集，SLI 除法整体消失 → SLO 序列与燃尽率告警一起静默失效。
  - **totalQuery 末尾 `(... > 0) OR on() vector(1)`（2026-08-12 踩坑）**: 零请求窗口里
    `rate()` 不是空集，是值为 0 的真实样本；配上上一条的 `vector(0)` 就成了
...
- **SLO 自身的哨兵** `SLOSLIProducingNaN`（`alerts/slo-meta-alerts.yaml`，2026-08-12 新增）:
  ...
  不是实数的 SLI 样本（NaN 参与任何比较都返回 false，故 `> -1` 会把它滤掉）。
...
- **⚠️ 零流量服务的 SLO 统计意义很弱**: 兜底后闲置窗口按"0 错误 = 健康"计入，序列不再断，
  ...
  **它根本不在监控清单里**（身份提供方竟是唯一没有存活探测的对外服务），已补上。
  ...
  判据见 [decisions/slo-availability-targets.md](../decisions/slo-availability-targets.md)
  （数字只在那里维护，这里不留副本）。
