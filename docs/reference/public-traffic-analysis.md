...
①和③对不上是正常的：差额 = 边缘缓存命中 + WAF 拦截 + Cloudflare 直接应答的部分。
...
`cf_infra`（Early Hints）携带的是真实访客的 IP（2026-08-14 有 927 个），
自建监控则只有 2 个 IP。也就是说 45% 的失真几乎全在请求数上、不在 IP 数上。
...
- ❌ `clientAsn` / `clientASNDescription`：**判断「这个 IP 属于 AWS/Hetzner 等数据中心
  还是住宅宽带」的最直接手段**。所以本仓库没有真正的「服务器 IP」判定，
...
采集侧的失效全是静默的（pod Running、探针绿、面板照常出图，只是数字停在几天前），
