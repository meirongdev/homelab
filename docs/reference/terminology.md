...
| 该写 | 别写 | 理由 |
|---|---|---|
| **控制面** | 控制平面 · master · 主节点 | 全仓 `控制面` 97 : `控制平面` 1。`master` 在本仓库另有含义（`master key`），且是 K8s 弃用术语 |
| **worker** | 工作节点 · 从节点 | 与节点名 `k8s-worker-106` 一致 |
| **宿主** / **宿主机** | 物理机 · 母机 | 指 `pve` / `storage-106` 这层 |
| **storage-106** | storage106 · 106 号机 | 带连字符才是主机名 |
| **106** | — | ✅ 可用作 `storage-106` **宿主**的简称。但**不要**用它指 worker 节点或那台 VM |
...
- 讲历史成因（大量 values 注释属于此类，如 "单节点无需反亲和"）→ 保留，但**新写**要加时间限定
- `plans/` 与 `decisions/` 是**冻结快照**（R1），里面的"单节点"不改也不算错
...
| 该写 | 别写 | 说明 |
|---|---|---|
| **ArgoCD** | Argo CD（带空格） | 上游官方拼法是 "Argo CD"，但本仓库统一 `ArgoCD`（455 : 0）。已成事实，不改 |
...
☠️ **`master` 单独说一下**：2026-08-18 全仓把 **73 处**指控制面节点的 `master` 改成了
`控制面`/`control-plane`。它是 K8s 弃用术语，而且在**本仓库**特别容易误读——同一个词还有
三个合法含义，都保留不动：
...
`scripts/check-terminology.py` 在 CI 跑（`docs-check.yml`），只查**机械可判、且判错就是事实
错误**的四条：

| 规则 | 查什么 |
|---|---|
| **T1** | 不存在的 context / 集群名（`homelab-k3s`、`k3s-oracle`…）—— 曾让脚本默认打错集群 |
| **T2** | 上表的拼写正典 |
| **T3** | `reference/`+`runbooks/` 里把 homelab 说成单节点（已带日期/时态限定的句子放过） |
| **T4** | `master` 指控制面节点（上表三种合法含义自动放过） |

它**故意不管**风格类变体（K3s vs K8s 的语义选择、App vs Application、`106` 简称）——
那些写成规则必然误报，误报多了整个检查就会被绕过，只能靠本文档 + review。
`docs/plans/` 与 `docs/records/` 整体豁免：按 R1 它们是冻结快照，改它反而抹掉历史。
