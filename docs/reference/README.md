...
- [tech-stack.md](tech-stack.md) — **技术栈全景**：每个组件解决什么问题、在这套系统里的特殊之处、
  配置与版本钉在哪（真相源地图）、以及故意没装什么。不熟悉这套栈从这里开始，也可当「我该去改哪个文件」查
...
- [public-traffic-analysis.md](public-traffic-analysis.md) — 公网访问分析**唯一真相源**：谁在访问哪个域名、来源分类法（真人/爬虫/自建监控）与可信度、☠️ 自建监控占请求 45%、PromQL 配方、免费版能力边界、按文章（path）的访问量与它的长期表
...
- [manifest-safety-checks.md](manifest-safety-checks.md) — CI 强制的清单规则 H1-H5 / V1-V5 / E1 + 渲染检查 + 「静态查不出、只能靠人」的那几类
