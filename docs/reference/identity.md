...
- **单一 IdP**: ZITADEL，`auth.meirong.dev`。**没有共享入口层 SSO**（Traefik ForwardAuth /
  ...
  公开、原生 ZITADEL OIDC、或自带认证（如 Vault、Timeslot admin Basic Auth）。
- **方向约定**: `HTTPRoute` 保持 controller-neutral，认证放应用层。优先原生 OIDC；
  应用不会说 OIDC 时才用 per-app `oauth2-proxy` 反代（当前无实例：Excalidraw 那个
...
- **部署机制是 k3s `HelmChart` CR，不是 ArgoCD Helm App**: 改 `valuesContent` → git push →
  ...
  所以手动 scale 其子 Deployment 不会被 selfHeal 拉回。
- **不入 git 的 bootstrap 依赖**: `zitadel` ns 里的 `login-client` Secret（Login V2 PAT）：
  setup job 在已恢复的 DB 上不会重建它，重建集群时要从备份拷入
...
- 各自的机密 WEB client 由 `zitadel/scripts/configure-oidc-app.sh` 幂等下发
...
- **ArgoCD**: dex 保持禁用，用原生 `configs.cm.oidc.config`，`clientID/clientSecret:
...
**Jellyfin** (`media`, homelab, 2026-08-16) 用原生 OIDC 插件，不是 oauth2-proxy 反代：

- **为什么是插件不是反代**：Jellyfin 有非浏览器客户端（iOS/Android/Kodi/Infuse + 直连
  流媒体 URL），走自己的 API token，不走浏览器 cookie 会话。在反代层套 oauth2-proxy 会
  把这些客户端/流 URL 全挡掉。正解是在认证层面接 OIDC：插件对接 ZITADEL 登录，
...
- **锁死注册**: `isCreationAllowed/isAutoCreation=false`、`autoLinking=AUTO_LINKING_OPTION_EMAIL`：
  陌生 GitHub 账号无法自助注册；GitHub 身份只能按已验证邮箱链接到既有 ZITADEL 用户。
