...
| 用户 Web 服务 | `500m` | calibre-web、miniflux、karakeep、rsshub |
| 数据库 | `500m` | postgres |
| 后台/轻量服务 | `200m` | gotify、homepage、uptime-kuma、redis、redpanda-connect |
...
Helm 管理的服务（Prometheus、Grafana、Loki、Tempo、ArgoCD、Vault、ZITADEL、Cilium）resources 在对应 `values/*.yaml` 中配置，均已设置完整的 CPU + Memory requests/limits。
