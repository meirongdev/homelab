...
☠️ **提交前跑 `check-docs.py` 对这一条是假绿**：判据是该文件最后一次**内容提交**的
日期，而未提交的改动没有提交日期 —— 所以工作区里改完就跑，STAMP 永远不报，
一 push 就红（2026-08-24 真这么红过一次）。要么改内容时顺手把 `Last updated`
改成今天，要么 `git commit` 之后再跑一次 `check-docs.py`、红了就 `--amend`。
