...
- **Dashboards 组织** (2026-06-15 整改，治理面板平铺混乱 + 跨集群指标叠加): Grafana 面板按文件夹分组，核心配置在 `k8s/helm/values/kube-prometheus-stack.yaml` 的 `grafana.sidecar.dashboards`：
  ...
  - **数据源固定**: Prometheus 类面板(multicluster / dgx)的 `datasource` 模板变量固定并隐藏(`hide:2`, 值 `prometheus` —— chart 的稳定 uid)。**不要给 Loki/Tempo 设显式 `uid:`**: 本集群 Grafana 用 NFS PVC 持久化，库里已有按 name 自动生成 uid 的 Loki/Tempo 数据源; provisioning 按 name 匹配，强行改 uid 会让 Grafana 12.x 报 `Datasource provisioning error: data source not found` 并整个 Grafana **Pod CrashLoop**(2026-06-15 踩坑，已回退)。故 Loki 类面板的 `datasource` 变量保持自动选择(集群内只有一个 Loki 数据源，会被自动选中)。
