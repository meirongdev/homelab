...
- **Managed by ArgoCD** (auto-sync + selfHeal; homelab in-cluster, plus oracle-k3s as an external cluster). **27 个 Application，全部在 `homelab` project 下**；核对: `kubectl -n argocd get app`：
...
- **NOT managed by ArgoCD** (manual `just` commands):
  ...
  - ~~kube-prometheus-stack~~ / ~~external-dns~~ — **已于 2026-07-31 迁入 ArgoCD**（见上）。`just deploy-prometheus` / `deploy-external-dns*` 保留为 `⚠️ LEGACY` 逃生通道，**日常不要跑**（selfHeal 会与手动 helm 来回打架）；紧急回滚仍可 `helm -n <ns> rollback <release> <rev>`
  - ⚠️ **OTel Collector：homelab 上根本没有部署**（2026-07-31 实测无 release 无 pod）。后果是 homelab 自己的容器日志没进 Loki（Loki 里只有 `cluster="oracle-k3s"`）；`just deploy-otel-collector` + `values/opentelemetry-collector.yaml` 是未落地的遗留物。要不要补部署是一次容量决策（5600H 单节点 + Loki PVC 增长），见 `docs/decisions/manual-helm-to-argocd-adoption.md` 决策四
