...
- **Current model**: services are either public, gated by **native ZITADEL OIDC** (see list below), or rely on their own built-in auth (for example ArgoCD, Vault, KaraKeep, and Timeslot admin Basic Auth).
...
- **Native ZITADEL OIDC apps**: **Stirling-PDF** (`pdf`), **Grafana** (`grafana`), and **Miniflux** (`rss`) speak OIDC directly — no oauth2-proxy. Each has a confidential WEB client provisioned (idempotently) by `zitadel/scripts/configure-oidc-app.sh` (REST, not Terraform — TF/gRPC writes break across the CF edge); creds live in Vault under the app's own path (`secret/homelab/grafana`, `secret/oracle-k3s/{stirling-pdf,miniflux}`, keys `oauth_client_id`/`oauth_client_secret`) → ESO → the app's K8s Secret. **Local username/password login is kept enabled as a fallback on each** (no lockout). Redirect URIs: Grafana `…/login/generic_oauth`, Miniflux `…/oauth2/oidc/callback`, Stirling `…/login/oauth2/code/oidc`. Grafana is deployed by `just deploy-prometheus` (Helm, **not** ArgoCD), so its values + ESO change need that command after the Vault write; Miniflux/Stirling reconcile via the `oracle-k3s` ArgoCD app on `git push`. **Grafana role mapping**: `role_attribute_path: "'Admin'"` grants Admin to any ZITADEL-authenticated identity (safe for this single-user, locked-down instance). **Miniflux**: `OAUTH2_USER_CREATION=1` auto-provisions on first SSO login; to keep admin rights, log in as the local admin first and link the OIDC identity under Settings.
