...
- **✅ DB migrated to CloudNativePG (2026-07-18)**: the bitnami PG above was replaced by a **CNPG `Cluster` CR `zitadel-pg`** (PG 17.6, single instance, Service `zitadel-pg-rw`) in the oracle `zitadel` ns; the operator comes from the `cnpg-operator` ArgoCD App (chart `cloudnative-pg` 0.29.0). Credentials reuse Vault `secret/homelab/zitadel` `db-password` (the app user shares the superuser password — deliberate, single-tenant DB). The old bitnami `zitadel-db` StatefulSet is deleted; the final pre-migration dump is kept off-repo at `~/backups/zitadel-migration/`. Operational gotchas:
  - ⚠️ **CNPG pods run `readOnlyRootFilesystem: true`** — `/tmp` is **not** writable. Scratch files must go to `/controller` (writable emptyDir) or a path under pgdata. Verified 2026-07-31: `touch /tmp/x` → `Read-only file system`, `/controller` OK.
  - ⚠️ **A backup container's `pg_dump` must be ≥ the server version** — client 16 refuses to dump a 17 server. This is why `backup/base/cronjob.yaml` pins `alpine:3.22` (ships `postgresql17-client`); see the comment there.
  - ZITADEL itself is still deployed by **k3s `HelmChart` CRs**, not an ArgoCD Helm App: edit `valuesContent` → git push → ArgoCD syncs the CR → the k3s helm-controller re-runs `helm upgrade`. ArgoCD only tracks the `HelmChart` CR, so manually scaling its child Deployments is **not** reverted by selfHeal.
...
- **Managed by ArgoCD** (auto-sync + selfHeal; homelab in-cluster, plus oracle-k3s as an external cluster). **28 个 Application，全部在 `homelab` project 下**；核对: `kubectl -n argocd get app`：
  - `root` App → `argocd/applications/` (App-of-Apps; manages all child Applications below)
  - **manifests/ 布局约定（2026-07-31）**：`k8s/helm/manifests/` 一个子目录 ↔ 一个 App（目录源，文件放进目录即生效；此前的 `directory.include` 文件清单已废除）。所有权地图见 `k8s/helm/manifests/README.md`，决策见 `docs/decisions/manifests-directory-per-app.md`
  - `personal-services` App → `manifests/personal-services/`（homelab；**只剩 open-notebook 全家 + LimitRange/Quota**——calibre 全家 2026-08-03 迁 oracle）
  - `gateway` App → `manifests/gateway/`（homelab Cilium Gateway；`gateway.yaml` = GatewayClass + Gateway 本体，每条对外路由一个 `route-<service>.yaml`（ReferenceGrant + HTTPRoute 成对）。新子域名 = 新建一个 `route-*.yaml`）
  - `cloudflare` App → `manifests/cloudflare/cloudflare-tunnel.yaml` (homelab)
  ...
  - `kube-bench` App → `manifests/kube-bench/kube-bench.yaml` (homelab CIS CronJob)
  - `namespace-guardrails` App → `manifests/namespace-guardrails/namespace-guardrails.yaml` (homelab LimitRange guardrails)
  ...
  - `external-dns` App（homelab，`external-dns` ns）+ `external-dns-oracle` App（**oracle-k3s 外部集群**）— **2026-07-31 chart 本体从 manual-helm 采纳进 ArgoCD**（多源：chart 1.21.1 + `$values`；homelab 那个同时管 `manifests/external-dns/` 里的 ExternalSecret）。⚠️ oracle 侧必须带 `helm.releaseName: external-dns` —— 详见下面 `kube-prometheus-stack` 那条的说明
  ...
  - `kube-prometheus-stack` App (Helm chart 87.6.0, ns `monitoring`, homelab) — **2026-07-31 从 manual-helm 采纳进 ArgoCD**。采纳前逐对象验证渲染等价（live 98 vs 渲染 105：0 裁剪、+7 全是跑完即删的 Helm hook、1 处 live-only 字段）。⚠️ 三条硬约束，改动前必读 `docs/decisions/manual-helm-to-argocd-adoption.md`：(1) **Application 名不能改**（= Helm release 名，会进 Deployment `spec.selector` 这个不可变字段；跨集群同 chart 用 `helm.releaseName` 解耦，`fullnameOverride` **治不了**采纳场景）；(2) `skipCrds: true` 是刻意的 —— 集群 CRD 停在 operator v0.89.0 而实际运行 v0.92.1（`helm upgrade` 从不升级 `crds/`），让 ArgoCD 接管会夹带一次 10 个 CRD 的升级，**该升级仍是待决项**；(3) admission webhook 的 `caBundle` 已豁免（运行时由 Job 注入，chart 侧为空）
  - `otel-collector` App (Helm chart `opentelemetry-collector` 0.165.0 / 镜像 `otel/opentelemetry-collector-k8s:0.156.0`, ns `monitoring`, homelab) — **2026-07-31 首次部署**（此前 homelab 根本没有 collector——无 release 无 pod，容器日志从未进 Loki）。容器日志→Loki（native OTLP）+ 应用 OTLP 追踪→Tempo；**刻意无 metrics 管道**（kps 原生抓取，含 cloudflared）。Loki 查询标签是 OTel 风格（`k8s_namespace_name`/`k8s_pod_name`/`cluster`，**不是** Prometheus 风格 `namespace`）。oracle 侧 collector 同日现代化（container operator 修 CRI 分段行 bug、file_storage checkpoint + 跨 Tailscale 持久发送队列、contrib 0.120→0.156——contrib 保留因 PRW exporter 不在 k8s 裁剪发行版里）。升级纪律：chart pin 与 oracle 镜像 tag 同 appVersion 一起动。见 `docs/decisions/otel-2026-alignment.md`
...
- **Current PVC inventory: everything is `local-path`.** Verified against **both** live clusters 2026-08-03（迁移收尾后重新生成；上一版停在 07-31，已含两处过期：仍列着随 Loki 迁走的 `storage-loki-0`，且漏掉 08-01/08-03 新增的两个 open-notebook 卷）。**There is no `nfs-client` StorageClass** — a PVC referencing it stays Pending forever.
  - **homelab（10 个）**: `data-vault-0`、`audit-vault-0`、`bifrost-data-local`、`data-trivy-server-0`、`alertmanager-…-db`、`prometheus-…-db`、`kube-prometheus-stack-grafana`、`opencost-pvc`、`open-notebook-data-local`、`open-notebook-surreal-local`
  - **oracle-k3s（14 个）**: `storage-loki-0`、`storage-tempo-0`、`opencost-pvc`、`calibre-books-local`、`calibre-web-automated-config-local`、`stirling-pdf-configs`、`timeslot-pvc`、`trends-data`、`uptime-kuma-data`、`karakeep-data`、`meilisearch-data`、`miniflux-db-pvc`、`data-trivy-server-0`、`zitadel-pg-1`
  - ⚠️ 这份清单**天然会漂移**（docs-check 只查结构，查不出内容与集群不符——2026-07-31 那次 NFS 描述就是格式完美而内容全错）。改集群存储后重新生成：`kubectl --context <ctx> get pvc -A`。
  - 「新增 PVC 却忘了纳入备份」这一类**已由 CI 拦截**（`scripts/check-manifests.py` 的 H4，见 [reference/manifest-safety-checks.md](reference/manifest-safety-checks.md)）——它上线即抓到 `trends-data` 静默未备份约两个月。
...
- **Migration history** (`docs/plans/storage/2026-07-06-storage-local-migration-and-backup-redesign.md`): the 2026-07-06 plan was a *tiered* design — move sqlite/PG/fsync victims to `local-path`, deliberately leave append-log/bolt/rebuildable volumes (`alertmanager-*-db`, `audit-vault-0`, `data-trivy-server-0`) plus the Calibre book library on NFS/ZFS. **That tiering no longer exists**: the 106 outage of 07-08→07-11 forced *all* remaining volumes onto `local-path` and the provisioner was uninstalled. Read the tiering rationale in that plan as history, not as current layout.
  ...
  - **Calibre 书库 is in restic.** The 07-06 plan said books were ZFS-only and excluded from restic (re-downloadable); that changed with the move to `local-path`. **2026-08-03 起书库在 oracle-k3s**，发现逻辑随之搬到 `backup/overlays/oracle/backup-script.yaml`（同样 discovers `/localpath/*calibre-books-local*`，缺失时打 `[warn] books NOT in this backup`）。⚠️ oracle 的备份脚本是**显式白名单**，新增有状态应用必须往里加，否则静默不备份。
...
- **Alerting** (Alertmanager → Telegram, 2026-07-18 起): `severity: warning|critical` rules route natively via Alertmanager `telegramConfigs`(零中间 bridge)到群 **MatthewDaily** 的「🚨 Homelab 告警」话题(`chatID: -1003981213530` + `messageThreadID: 2`); `info` is dropped. **`Watchdog` is NOT dropped** — the `watchdog` AlertmanagerConfig CRD claims it first and webhooks it to the oracle Uptime Kuma push monitor (dead-man's switch, see below); the static tree's `null` receiver only catches genuinely unmatched alerts. bot token: Vault `secret/homelab/telegram` → ESO(`alertmanager-telegram-secret.yaml`)。**New `PrometheusRule`/`ServiceMonitor` resources MUST carry the label `release: kube-prometheus-stack`** or the operator's `ruleSelector`/`serviceMonitorSelector` ignores them silently. First rule: **ESO health** (`eso-alerts.yaml`, deployed via the ArgoCD `monitoring-dashboards` Application). A single rule covers both clusters since oracle ESO metrics arrive remote-written with `cluster=oracle-k3s`. 旧的 `alertmanager-gotify-bridge`(druggeri/alertmanager_gotify_bridge)已下线——有 `concurrent map writes` fatal 崩溃 bug(全局无锁 metrics map，两个并发 webhook 即崩，50 天 66 次重启）且上游无维护。**Gotify 本体已于 2026-07 彻底下线**（Falco 告警、dead-man's switch 均已改走原生 Telegram，RSS 阅读推送直接砍掉未迁移；Deployment/PVC/网关路由/DNS/homepage/SLO/backup 条目全部移除）。详见 `decisions/alerting-telegram-migration.md`。
  - **⚠️ 告警覆盖 ≠ 抓取覆盖（2026-08-02 核实）**: kube-prometheus-stack 的 node-exporter mixin 规则（`NodeMemoryHighUtilization` / `NodeFilesystemAlmostOutOfSpace` / CPU 那一组）**全部硬编码 `job="node-exporter"`**，因此**只覆盖两个 K3s 节点**（`10.10.10.10` / `10.0.0.26`）。上面列的裸机与外部主机跑在**别的 job 名**下——`node-exporter-metal-nodes`（pve + storage-106）、`node-exporter-dgx-spark`、`node-exporter-macbook`——mixin 规则一条都不匹配它们。这些主机当前**只有**自建的 `ProxmoxHostDown` / `BackupTargetNodeDown`（都仅 `up==0`）与 SMART/ZFS 那组（`storage-alerts.yaml`）：**没有内存 / 文件系统 / CPU 告警**，承载整个 homelab 集群的 hypervisor 也在内。实测症状：pve 读数 `MemAvailable` 4.4% 而 `NodeMemoryHighUtilization` 恒为 `inactive`（阈值 >90% for 15m，本该触发），纯粹是 job 标签不匹配。**已于 2026-08-02 补齐**（`prometheus-rules.yaml` 的 `metal-nodes-resources` 组）：刻意**没有**统一放宽成 `job=~"node-exporter.*"`，因为几台机的正常态差别太大，一个阈值必然要么误报要么漏报——改成按主机分别定阈值。两个实测结论值得记住：
    - **pve 的 ZFS ARC 实测约等于 0**（`node_zfs_arc_size` ≈ 0），它那 4.4% 的低 `MemAvailable` 是**虚拟机分配**（K3s VM 独占 12.6GiB / 共 15GiB）而非可回收缓存，而且已在用 1.07G swap。所以 pve 用**绝对下限**（<300Mi）而不是百分比——百分比阈值在 pve 上是永久误报。
    - **106 正好相反**：ARC 3.8G 是可回收的，`MemAvailable` 读数 1.4G 严重低估，加回 ARC 后真实可用 4.98G/7.57G ≈ 66%。所以 106 的表达式必须 `+ node_zfs_arc_size - node_zfs_arc_c_min`。
  - **⚠️ 采集能力不齐，写新规则前先确认指标存在**（否则写出永不触发的死规则）：**DGX ×2 没有 `node_filesystem_*`**（容器化 node_exporter 未挂宿主根 → 磁盘只能靠 SMART，见上文 nv-dgx-spark 部署方式）；**macbook 没有 `node_memory_MemAvailable_bytes`**（darwin 无此指标）。查法：`count by (job) (node_filesystem_size_bytes)`。
...
- **SLI / SLO** (2026-06-16 上线；2026-07-12 扩展至 oracle 网关): 服务可用性 SLO 基于**一手的 Cilium Gateway Envoy L7 指标**(真实入口请求，非合成探测)，用 **Sloth** 生成规则。
  ...
  - **Sloth**: ArgoCD `sloth` App（Helm chart + `values/sloth.yaml`，2026-07-06 迁入 ArgoCD——旧文说 `just deploy-sloth` 非 ArgoCD 已过时）。`sloth.extraLabels.release=kube-prometheus-stack` 让生成的 PrometheusRule 被 operator 的 ruleSelector 选中; `defaultSloPeriod=30d`; 关掉 commonPlugins 的 git-sync sidecar。CRD `PrometheusServiceLevel` 由 chart 安装。Sloth 与规则评估都在 homelab 侧（oracle 指标已 remote-write 过来）。
  - **SLO 定义**: `manifests/monitoring/slos.yaml`——**两个** `PrometheusServiceLevel`(ArgoCD `monitoring-dashboards` App 管理): `homelab-gateway-availability`(grafana/vault/bifrost) + `oracle-gateway-availability`(zitadel/**calibre-web/argocd**——后两个于 2026-08-02/03 随工作负载迁入；原 gotify-availability 已随 Gotify 下线移除)，共 6 个服务 99%/30d(error=5xx, total=全部类)。**新增/改服务或目标**: 在对应 `spec.slos[]` 追加一条(`errorQuery`/`totalQuery` 用 `envoy_cluster_name=~".*/<ns>_<svc>_.*"` 正则匹配路由；oracle 侧记得用 `_total` 指标名 + `cluster="oracle-k3s"`) + 改 `objective`，`git push` 即可。
...
- **保护层次**: ZFS raidz1（容 1 盘）→ sanoid 快照（秒级回滚、含 restic dataset）→ restic 仓库（护 local-path 关键数据）→ **离站 later**（rclone/`restic copy` → OCI always-free/B2，需人工开云桶）。书库 2026-07-11 起**也纳入 restic 夜备**（脚本自动发现 `/localpath/*calibre-books-local*`，缺失时日志打 `[warn] books NOT in this backup`）——**2026-08-03 起该逻辑在 oracle overlay**，homelab 侧已移除；PVE 每周 vzdump（VM 100 → 106，keep-last=3）不变。
- **告警**: `BackupTargetNodeDown`（106 失联 >15m，severity=**warning**）——2026-07-12 由 `NFSStorageNodeDown`(critical/2m) 改名降级：106 已无 NFS 运行时依赖（PVC 全部 local-path），宕机只影响备份窗口不影响在线服务。规则在 `manifests/monitoring/alerts/prometheus-rules.yaml`。
