...
- **Managed by ArgoCD** (auto-sync + selfHeal; homelab in-cluster, plus oracle-k3s as an external cluster). **25 个 Application，全部在 `homelab` project 下**；核对: `kubectl -n argocd get app`：
  ...
  - `gateway` App → `manifests/gateway/gateway.yaml` (homelab Cilium Gateway)
  ...
  - `external-dns` App → `manifests/external-dns/external-dns-secret.yaml`（homelab，`external-dns` ns）— 只管 ExternalSecret，Helm release 本身是 manual-helm（见下）
