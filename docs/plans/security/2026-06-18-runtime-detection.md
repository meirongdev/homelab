...
- **oracle → Falco + Falcosidekick**：OCI VM CPU 余量大；Falco 社区规则开箱即用，Falcosidekick 原生 Telegram output（gotify 已退役 2026-07）。
...
| Falco ns + Telegram token | `cloud/oracle/manifests/falco/{namespace.yaml,falcosidekick-secret.yaml}`（kustomize 拥有） | oracle |
...
2. **Falco 双出口**：JSON→stdout→Loki（always-on，零依赖）+ Falcosidekick→Telegram（原生 output，2026-07 起替代 Gotify）。`driver:modern_ebpf` CO-RE 免内核模块。
...
```bash
# 1) Telegram bot token（2026-07 起原生 output，替代 Gotify）
# 2) 写入 Vault（oracle 侧 ExternalSecret 跨集群读取该 homelab 路径）
vault kv put secret/homelab/telegram bot_token=<token>
```
...
# 触发 Falco 内置规则（如容器内 `cat /etc/shadow`）应在 Loki 看到+Telegram 收到告警
```

## 后续调优（路线图）

Tetragon 写 TracingPolicy（敏感文件/提权/异常外联）+ 基于其指标的告警；Falco 噪声规则按本环境裁剪（`customRules`）。
