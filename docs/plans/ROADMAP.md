...
- [x] **manual-helm → ArgoCD 采纳** ✅ 2026-07-31：`kube-prometheus-stack` + `external-dns` ×2 三个 Application 上线，改 values → push → 自动同步；justfile 对应配方标 `⚠️ LEGACY` 保留为逃生通道。采纳前逐对象验证渲染等价性（`helm get manifest` vs `helm template`，只在「0 裁剪、变更可解释」时动手）。取舍与三个陷阱见决策记录 [`decisions/manual-helm-to-argocd-adoption.md`](../decisions/manual-helm-to-argocd-adoption.md)，要点：
  ...
  - ❌ **otel-collector 未迁移，因为它根本没部署**：homelab 无 release 无 pod，`just deploy-otel-collector` + `values/opentelemetry-collector.yaml` 是未落地的遗留物（后经查旧 values 写的 `otlp_http` exporter 名根本不存在，配置从未跑通过）。连带发现 **homelab 自己的容器日志从未进 Loki**（Loki 里只有 `cluster="oracle-k3s"`）。✅ **同日解决**——见下面「OTel 2026 对齐」条目。
- [x] **OTel 2026 对齐** ✅ 2026-07-31：homelab collector 首次落地（ArgoCD `otel-collector` App：chart 0.165.0 + `otel/opentelemetry-collector-k8s:0.156.0` 裁剪发行版；日志→Loki、OTLP 追踪→Tempo；**刻意无 metrics 管道**，kps 原生抓）；oracle collector 现代化（`container` operator 取代 60 行手写解析链并修复 CRI 分段长行 bug、`file_storage` checkpoint + 跨 Tailscale 持久发送队列、contrib 0.120→0.156、+`k8s.cluster.name` semconv）；新增 `OracleTelemetryPipelineDown` 断流告警（oracle 全部遥测过一根管子，此前死了没人知道）。双侧配置经 0.156.0 **真实二进制 validate** 后才推送。采纳/拒绝清单见 [`decisions/otel-2026-alignment.md`](../decisions/otel-2026-alignment.md)。⚠️ Loki 查询标签是 OTel 风格（`k8s_namespace_name` 等，不是 `namespace`）。
...
- [x] **Vault 孤儿 secret 清理** ✅ 2026-07-18：全 Vault 枚举 × 两集群全部 ExternalSecret 交叉核对（且确认无 Vault Agent Injector，全走 ESO），删除 4 个确认无消费者的孤儿——`secret/homelab/postgres`（bitnami PG 退役遗物）、`secret/homelab/zitadel-oidc`（旧 SSO 共享 OIDC client）、`secret/oracle-k3s/oauth2-proxy`（同批遗留，client-id 与 zitadel-oidc 相同，核查确认亦为孤儿、oracle 无 oauth2-proxy 工作负载）、`secret/homelab/kopia`（2026-07-05 已软删，本次 metadata 彻底清除）。均 `DELETE /v1/secret/metadata/<path>` 全版本销毁。删后重新枚举确认剩余 16 个 path 全部有对应 ExternalSecret 消费者。
  ...
  - 本地明文清单 `k8s/helm/values/vault_values.md`（gitignored）**已删除 ✅ 2026-07-31**。⚠️ 本条此前（`ff2581d`，07-18）就写成"已删除"，但**文件其实一直在磁盘上**（mtime 停在 2026-03-07，从未被裁剪也从未被删）——它是 gitignored 的，删除动作不产生任何 diff，所以两次文档声明都没人发现落空了。07-31 实际执行了 `rm` 并复核确认文件不存在。
    删除前的核对（从集群侧枚举两集群全部 ExternalSecret 的 `remoteRef.key`，不需要 Vault token）：dump 里 16 条路径 **6 条仍活**（`homelab/{github,grafana,zitadel}` + `oracle-k3s/{cloudflare,stirling-pdf,uptime-kuma}`）、**10 条已死**（4 条是 07-18 主动销毁的孤儿 + gotify + 5 条已迁 oracle 的旧 homelab 路径），与当时的判断一致；且它**反过来缺 12 条当前在用的路径**（restic/telegram/argocd-oidc/argocd-oracle-cluster/homelab-cloudflare/external-dns/bifrost-oauth2-proxy/excalidraw-oauth2-proxy/karakeep/miniflux/trends/oracle-miniflux），当索引也已经不可用。删它不丢任何东西：活的 6 条 Vault 里都在（值可能比 dump 更新），Vault 本身是真相源且有 restic 夜备。
    `.gitignore` 规则 `**/vault_values.md` 保留（重生成物仍会被忽略），但**不再维护常驻明文副本**——需要临时清单时按需生成、用完即删。
