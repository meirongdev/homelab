...
- [~] **external-dns (Gateway API source)**: ✅ 2026-07-19 上线并端到端验证（`external-dns` App，`gateway-httproute` source，`homelab-gateway` 打 tunnel target 注解，policy=upsert-only 安全默认）。采用理由/取舍见决策记录 [`decisions/external-dns-adoption.md`](../decisions/external-dns-adoption.md)。**未完成部分**：`argocd`/`book`/`grafana`/`llm`/`vault` 这 5 条现有记录仍在 terraform 手管，尚未把归属权转给 external-dns、也未精简 `cloudflare/terraform/terraform.tfvars`——新加子域名目前仍可以走 HTTPRoute 单文件（有 external-dns 接管），但老的还是两步走并存。顺带发现 homelab 自己的 `cloudflare/terraform/terraform.tfvars` 的 `cloudflare_api_token` 是无效值（tunnel token 格式），该项目 `terraform plan` 现在直接报错，待修
