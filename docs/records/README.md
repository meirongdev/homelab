...
| 日期 | 记录 | 内容 |
|------|------|------|
| 2026-09-01 | [oracle-apparmor-af-unix-panic](2026-09-01-oracle-apparmor-af-unix-panic.md) | oracle 三个月的「宿主层硬重置」实为**内核 AppArmor 空指针**：`unix_fs_perm()` 不检查 `path->mnt` 就 `mnt_idmap()`（vfsmount 偏移 0x18，与故障地址/指令逐位吻合）。☠️ 客场永远查不到是因为 `panic_on_oops=1` 编译期默认，journald 来不及落盘 —— 「日志戛然而止无 panic」恰是 guest panic 的标准形态，不是宿主层证据。证据在 `oci compute console-history`：10 次 panic 签名 10/10 一致，肇事者全是 `libuv-worker`。触发方经 ftrace kprobe 定位为 **uptime-kuma 容器内默认自启的 nscd**（AppArmor 每次操作查两遍：自己的套接字 + **对端的**，所以带路径的那侧在 nscd，崩溃却落在客户端 libuv-worker），关掉后该函数调用 69→**0**。另含：OCI 维护事件最后一条在 3 月洗清宿主层 · **换内核逃不掉**（6.8/6.14/6.17/7.0 全带该代码，上游主线亦未修）· soft lockup 的 cilium-agent 是后果不是原因 · `set_settings` 会把没传的键重置成默认值 |
