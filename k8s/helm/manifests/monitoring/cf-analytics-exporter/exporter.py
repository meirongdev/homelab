...
指标（全部 gauge，**不带 `_total` 后缀**：它们是每日快照，不是单调计数器）:
  ...
  cf_analytics_daily_client_ips{host,date}    按域名的**独立客户端 IP 数**（同上有 __total__）
...
见: docs/reference/cloudflare-tunnel-observability.md
"""
...
# 单日 (clientIP, host) 组合数上限。CF 硬上限 10000；2026-08-15 实测本 zone 约 2500 行。
...
# 按 (clientIP, host) 分组：请求数直接求和，独立 IP 数按 host 去重后计数。
  ...
  viewer {
    zones(filter: {zoneTag: $zone}) {
      httpRequestsAdaptiveGroups(
        ...
        dimensions { clientIP clientRequestHTTPHost }
        count
      }
    }
  }
...
def fetch_host_day(day: str):
    """某一完整 UTC 日的 (请求数, 独立 IP 数) —— 按 host，外加 __total__ 汇总。"""
    rows = _zone_rows(
    ...
    for r in rows:
        ...
        requests[host] = requests.get(host, 0) + r.get("count", 0)
        ips.setdefault(host, set()).add(r["dimensions"]["clientIP"])

    requests[TOTAL] = sum(requests.values())
    # 全站独立 IP 要跨 host 再去重一次（同一个 IP 常常访问多个子域）。
    ips[TOTAL] = set().union(*ips.values()) if ips else set()
    return requests, {h: len(s) for h, s in ips.items()}, len(rows) >= ROW_LIMIT
...
def refresh() -> None:
    ...
    for offset in range(1, HOST_WINDOW_DAYS + 1):
        day = utc_date(offset)
        try:
            requests, ips, truncated = fetch_host_day(day)
        ...
        data["truncated"] |= int(truncated)
        for host, cnt in requests.items():
            data["requests"][(day, host)] = cnt
        for host, cnt in ips.items():
            data["client_ips"][(day, host)] = cnt
